...
// Instead of using the SCORM player, create a direct content launcher
// that loads the SCORM content in a minimal iframe without any Moodle navigation

// Get the context for file access
$context_module = \context_module::instance($cmid);

// Create a direct URL to the SCORM content file
// This bypasses the player interface entirely
$direct_content_url = new \moodle_url('/mod/scorm/loadSCO.php', [
    'id' => $cmid,
    'scoid' => $sco->id
]);
...
<body>
    ...
    <iframe id="scorm-frame" 
            ...
            src="<?php echo $direct_content_url; ?>" 
            frameborder="0"
            style="display: none;"
            onerror="showError()">
    </iframe>
